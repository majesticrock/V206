...
\\ \\
\centerline{$T'_i = 2a_it + b_i$     mit i = 1, 2}
\\ \\
bestimmen. 
...
\\ \\
Diese Werte werden in \eqref{eqt:wärme_zeit} \eqref{eqt:güte} eingsetzt, um so die Güteziffern $\nu$ zu errechnen. 
Dieser Fehler $\sigma_\nu$ dieses lässt sich ebenfalls mithilfe von Gleichung \eqref{eqt:fehlerfortpflanzung} berechnen.
\\ \\
\centerline{$\sigma_\nu^2 = 1 \cdot \sigma_\text{$T'_1$}^2 + \bigg(\frac{\symup{d}T_1}{\symup{d}t}\bigg)^2 \sigma_\text{$C_w$}^2$}
\\ \\
...
\begin{table}
  ...
  \begin{tabular}{
    ...
    \toprule
    \multicolumn{2}{c}{$T_1$ / K} & \multicolumn{2}{c}{$P_\text{mech}$ / W} \\
    \midrule
    301.3 & 0.1 & 36.170 & 0.012 \\
